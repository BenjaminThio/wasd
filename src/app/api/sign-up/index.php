<?php
    ...
    if (!preg_match('/^[A-Za-z0-9_-]+$/', $username)) {
        Api::fail('Usernames can only contain letters, numbers, underscores and hyphens.', 422);
    }

    if (!filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($email) > 191) {
        Api::fail('Enter a valid email address.', 422);
    }
    ...
    if (Users::emailTaken($email)) {
        Api::fail('An account with that email already exists.', 409);
    }
